...
class onerun:
    ...
    #                 y_true.extend(input["label"].tolist())
    #                                     # Identify incorrect predictions and store their indices
    #                 batch_indices = batch['index']  # Assuming the dataloader provides an 'index' key for IDs
    #                 incorrect_mask = preds != input["label"]  # Boolean mask for wrong predictions
    #                 wrong_indices.extend(batch_indices[incorrect_mask].tolist())
    ...
    def eval(self, mode, epoch=None):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.dataloaders[mode]):
                ...
                try:
                    ...
                    # If there's an exception, replace input["img"] with y_pred
        ...
        # Save predictions
        if epoch:
            np.savez(self.predict_save_path[:-4] + str(epoch) + '.npy',
                    y_pred=np.array(y_pred),
                    y_true=np.array(y_true),
                    score=np.array(scores_list))
            np.save(self.predict_save_score_path[:-4] + str(epoch) + '.npy',
    ...
    with torch.no_grad():
        for i, batch in enumerate(self.dataloaders[mode]):
            ...
            try:
                ...
                # Handle exception and replace input["img"]
                print(f"Exception occurred in batch {i}: {e}")
                input["img"] = y_pred
                continue  # Skip processing this batch
